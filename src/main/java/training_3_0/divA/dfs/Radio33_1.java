...
public class Radio33_1 {
    private static int[][] color;
    private static List<Integer>[] adjList;
    private static int INF = 1000000;
    private static int[][] points;
    private static int globalLenght;

    private static void dfs(int i, int v, int c, int last) {
        color[i][v] = c;
        for (Integer u : adjList[v]) {
            if (color[i][u] == 0) {
                if(last!=-1){
                    globalLenght = Math.min(globalLenght, (points[last][0] - points[u][0]) * (points[last][0] - points[u][0]) + (points[last][1] - points[u][1]) * (points[last][1] - points[u][1]));
                }
                dfs(i, u, 3 - c, v);
            }
        }
